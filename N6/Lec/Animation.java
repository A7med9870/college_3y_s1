...
public class Animation {
    ...
    /**
     * Loads a subset of images from a sprite sheet into this animation.
     * 
     * @param fileName The path to the file to load the animations from
     * @param columns	How many columns there are in the sheet
     * @param rows		How many rows there are in the sheet
     * @param frameDuration	The duration of each frame
     * @param from		The index of the start frame counting across a row and then down (initial frame is 0)
     * @param numFrames		The number of frames to load from the initial position
     */
    public void loadAnimationSeries(String fileName, int columns, int rows, long frameDuration, int from, int numFrames)
    {
    	ArrayList<Integer> frameIndex = new ArrayList<>();
    	for (int i = from; i<from+numFrames; i++) frameIndex.add(i);
    	ArrayList<BufferedImage> imagesList = loadSubsetFromSpriteSheet(fileName, columns, rows, frameIndex);
    	
    	for (BufferedImage img : imagesList) addFrame(img,frameDuration);
    }
    
    /**
     * Loads a subset of images from a sprite sheet into an ArrayList.
     * 
     * @param spritesheetPath Path to the sprite sheet image file
     * @param framesPerRow Number of frames per row in the sprite sheet
     * @param framesPerColumn Number of frames per column in the sprite sheet
     * @param frames List of frame indices (starting at 0) to load
     * @return ArrayList containing the loaded images
     */
    private ArrayList<BufferedImage> loadSubsetFromSpriteSheet(
        ...
        ArrayList<Integer> frames) 
    {
    	...
    	try
    	{
	        ...
	        int frameWidth = spritesheet.getWidth() / framesPerRow;
	        int frameHeight = spritesheet.getHeight() / framesPerColumn;
	        
	        
	        for (Integer frameIndex : frames) {
	            // Calculate the row and column for the current frame
	            int row = (frameIndex % framesPerRow);
	            int col = (int)(frameIndex / framesPerRow);
	            
	            // Calculate the x and y coordinates to crop from the spritesheet
	            int x = row * frameWidth;
	            int y = col * frameHeight;
	            
	            // Don't attempt to load a subimage outside the bounds of the image
	            if (x + frameWidth > spritesheet.getWidth() || y + frameHeight > spritesheet.getHeight()) continue;
	            // Extract the sub-image
	            BufferedImage image = spritesheet.getSubimage(x, y, frameWidth, frameHeight);
    	...
    	{
    		System.err.printf("Failed to read from sprite sheet file: %s",e);
    	}
