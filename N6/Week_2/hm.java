...
public class hm extends JFrame {
    ...
    public void go() {
        ...
        car = new ImageIcon("Screenshot_573.png").getImage();
    ...
    public void drawOmanFlag(Graphics g, int x, int y, int width, int height) {
        ...
        // Red stripe (middle)
        g.setColor(Color.RED);
        g.fillRect(x + width / 4, y + stripeHeight, width - width / 4, stripeHeight);

        // Green stripe (bottom)
        g.setColor(Color.GREEN);
        g.fillRect(x + width / 4, y + 2 * stripeHeight, width - width / 4, stripeHeight);
    }
}
}
